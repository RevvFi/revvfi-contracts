...
contract RewardsDistributor is ReentrancyGuard, AccessControl {
    ...
    event DistributionUpdated(
        uint256 distributedSinceLastUpdate,
        uint256 cumulativeDistributed
    );
    ...
    function updateEmissionRate(uint256 newEmissionRate) external onlyGovernance scheduleExists whenNotPaused {
        ...
        require(newEmissionRate <= maxIncrease && newEmissionRate >= maxDecrease,
        ...
        whenNotPaused 
    {
        ...
        require(token.balanceOf(address(this)) >= getRemainingTokens() + additionalTokens,
     ...
     */
    function getDistributionSchedule() external view returns (
        uint256 startTime,
        uint256 endTime,
        uint256 totalAllocation,
     ...
     */
    function getClaimerInfo(address claimer) external view returns (
        uint256 lastClaimTime,
        uint256 pendingRewards,
        bool active
    ) {
        return (
            claimers[claimer].lastClaimTime,
            getClaimableRewards(claimer),
            claimers[claimer].active
        );
    }
