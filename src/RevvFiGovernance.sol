...
contract RevvFiGovernance is ReentrancyGuard, AccessControl {
    ...
    function propose(
        ...
        emit ProposalCreated(proposalCounter, msg.sender, target, proposalType, 
            block.timestamp, block.timestamp + VOTING_PERIOD, description);
    ...
    function vetoProposal(uint256 proposalId) external onlyCreator proposalExists(proposalId) {
        ...
        require(proposal.state == PROPOSAL_STATE_SUCCEEDED, 
            "RevvFiGovernance: proposal not in succeeded state");
        require(!proposal.executed, "RevvFiGovernance: already executed");
    ...
    function getVoteResults(uint256 proposalId) external view returns (
        ...
        uint256 quorumRequired = (proposal.totalVotingPowerAtStart * _getQuorumThreshold(proposal.proposalType)) / BASIS_POINTS;
